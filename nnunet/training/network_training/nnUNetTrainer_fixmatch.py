...
class nnUNetTrainerFixmatch(NetworkTrainerFixmatch):
    def __init__(self, plans_file, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.basic_generator_patch_size_weak = None
    ...
    def get_basic_generators(self):
        ...
        if self.threeD:
            ...
            dl_val = DataLoader2D(self.dataset_val, self.patch_size, self.patch_size, self.batch_size,
                                  oversample_foreground_percent=self.oversample_foreground_percent,
                                  pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
            dl_test = DataLoader2D(self.dataset_test, self.patch_size, self.patch_size, self.batch_size,
                                   oversample_foreground_percent=self.oversample_foreground_percent,
                                   pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
            dl_tr_un = DataLoader2D(self.dataset_unlabeled, self.patch_size, self.patch_size,
                                    int(self.batch_size) // 2,
                                    oversample_foreground_percent=self.oversample_foreground_percent,
                                    pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
        return dl_tr, dl_tr_un, dl_val, dl_test
    ...
    def test(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
             save_softmax: bool = True, use_gaussian: bool = True, overwrite: bool = True,
        ...
        # predictions as they come from the network go here
        output_folder = join(self.output_folder, test_folder_name)
        maybe_mkdir_p(output_folder)
        # this is for debug purposes
        my_input_args = {'do_mirroring': do_mirroring,
                         'use_sliding_window': use_sliding_window,
                         ...
                         'use_gaussian': use_gaussian,
                         'overwrite': overwrite,
                         'validation_folder_name': test_folder_name,
                         'debug': debug,
                         'all_in_gpu': all_in_gpu,
                         'segmentation_export_kwargs': segmentation_export_kwargs,
                         }
        ...
        if run_postprocessing_on_folds:
            ...
            # have this applied during inference as well
            self.print_to_log_file("determining postprocessing")
            determine_postprocessing(self.output_folder, self.gt_niftis_folder, test_folder_name,
                                     final_subf_name=test_folder_name + "_postprocessed", debug=debug)
            # after this the final predictions for the vlaidation set can be found in validation_folder_name_base + "_postprocessed"
            # They are always in that folder, even if no postprocessing as applied!
